		...
		_hyperscript.addFeature("socket", function (parser, runtime, tokens) {
			if (tokens.matchToken("socket")) {
				...
				if (tokens.matchToken("on")) {
					...
					messageHandler = parser.requireElement("commandList", tokens);
					var implicitReturn = {
						type: "implicitReturn",
						op: function (context) {
							return runtime.HALT;
				...
				function connect() {
					if (featureTarget && (!socket || socket.readyState > 1)) {
						runtime.triggerEvent(featureTarget, "reconnecting", {});
					}
					...
					var newSocket = createSocket(url);
					...
					newSocket.addEventListener("close", function (e) {
						socket = null;
					});
					socket = newSocket;
					socketObject.raw = newSocket;
				}
							...
							get: function (obj, property) {
								if (PROXY_BLACKLIST.indexOf(property) >= 0) {
									...
									return function () {
										...
										if (!socket || socket.readyState > 1) { // 2=CLOSING, 3=CLOSED
											connect();
										}

										var dataToSend = JSON.stringify(rpcInfo);
										if (socket.readyState === 0) { // CONNECTING
											sendQueue = [dataToSend];
										} else { // OPEN
				...
				rpcProxy = getProxy(defaultTimeout);
				...
				Object.assign(socketObject, {
					...
					dispatchEvent: function (evt) {
						...
						var dataToSend = JSON.stringify(Object.assign({ type: evt.type }, details));
						if (socket.readyState === 0) { // CONNECTING
							sendQueue = [dataToSend];
						} else { // OPEN
							socket.send(dataToSend);
