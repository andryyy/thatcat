...
_hyperscript.addCommand('sort', function(parser, runtime, tokens) {
    if (!tokens.matchToken("sort")) return;

    var sortables = parser.requireElement("expression", tokens);

	var operationToken = tokens.matchToken("by");

    var sorting = parser.requireElement("expression", tokens);

    if (sorting.type != "symbol" && sorting.type != "attributeRef") {
        return;
    }
    ...
    var sortCmd = {
        value: sortables,
        args: [sortables],
        op: function (context, sortables) {
            if (sortables == null) {
                sortables = [];
            } else if (!(Array.isArray(sortables) || sortables instanceof NodeList)) {
                sortables = [sortables];
            }

            unsortedGroup = []
            sortables.forEach(function (part, idx) {
				unsortedGroup[idx] = []
            	sortables.forEach(function (potentialSibling) {
            		if (part.parentNode == potentialSibling.parentNode) {
            			positionInParent = Array.prototype.indexOf.call(part.parentNode.children, potentialSibling)
						unsortedGroup[idx].push(potentialSibling)
						if (sorting.type === "attributeRef") {
							console.log(sorting.name)
							console.log(potentialSibling.getAttribute(sorting.name))
						}
						if (sorting.type === "symbol") {
							console.log(potentialSibling[sorting.name])
						}
            		}
